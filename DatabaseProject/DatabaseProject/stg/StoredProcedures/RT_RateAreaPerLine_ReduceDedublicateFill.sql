...
create procedure [stg].[RT_RateAreaPerLine_ReduceDedublicateFill]
as
begin
...
begin
    ...
    (
        ...
        from
            [stg].[RT_RateAreaPerLine] ra1
            join [stg].[RT_RateAreaPerLine] ra2 on
                ra2.[LineNumber] = ra1.[LineNumber] 
                and ra2.[RateAreaCode] = ra1.[RateAreaCode]
...
(
    select
        [R] = row_number() over (partition by ra.[LineNumber], ra.[RateAreaCode], ra.[ValidFromDate] order by isnull(ra.[ValidToDate], '9999-12-31') desc)
    from
        [stg].[RT_RateAreaPerLine] ra
)
delete from
    [_Dublicated]
where
...
(
    select
        [ValidToDate],
        [_ValidToDate] = lead(dateadd(day, -1, [ValidFromDate])) over (partition by [LineNumber], [RateAreaCode] order by [ValidFromDate])
    from
        [stg].[RT_RateAreaPerLine] 
)
update
