...
create view [data].[ACCT_Settlement_VehicleJourneyAssignment_Monthly]
as
with
[PT] as
(
    ...
    from
        [data].[ACCT_Settlement_PTVehicleJourneyAssignment_Monthly] as [A]
    where
        ...
        from
            [data].[ACCT_Settlement_PTVehicleJourneyAssignment_Monthly] as [B]
        where
...
(
    ...
    from
        [data].[ACCT_Settlement_RKVehicleJourneyAssignment_Monthly] as [A]
    where
        datediff(second, [AssignedFromDateTime], [AssignedToDateTime]) = (
        select
            [MaxDuration] = max(datediff(second, [AssignedFromDateTime], [AssignedToDateTime]))
        from
            [data].[ACCT_Settlement_RKVehicleJourneyAssignment_Monthly] as [B]
        where
            [A].[JourneyRef] = [B].[JourneyRef]
        group by
            [JourneyRef]
