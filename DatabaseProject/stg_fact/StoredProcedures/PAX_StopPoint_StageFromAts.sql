...
-- Description: Stages measures in stg_fact.PAX_StopPoint for a given period based on imputated measurements from ATS.
-- =============================================
CREATE procedure [stg_fact].[PAX_StopPoint_StageFromAts] (@Period int)
as
begin
    ...
    select
       ...
       ,s.[ImputationRule]
       ,s.[BoardingWeightAvg]
       ,s.[AlightingWeightAvg]
    from
        [data].[APC_ATS_TimeGroupUniqueJourneyPatternPointImputed] s
        join [MonthDistribution] dist on
            dist.[OperatingDayType] = s.[OperatingDayType]
    ...
    where
        ...
        -- Published data only
         s.[Published] = 1
        -- DayType 1 -> Hverdag. Eq. of @DayTypeGroup = 1 -> DayType = 1
        and (not @DayTypeGroup = 1 or s.[OperatingDayType] = 1)
        -- DayType 2, 3 -> Lør, Søn og Helligdag. Eq. of @DayTypeGroup = 2 -> DayType in (2, 3)
        and (not @DayTypeGroup = 2 or s.[OperatingDayType] in ( 2, 3 ))
        and s.[Period] = @Period
