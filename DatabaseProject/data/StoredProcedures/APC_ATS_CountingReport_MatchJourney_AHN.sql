...
-- =============================================
create procedure [data].[APC_ATS_CountingReport_MatchJourney_AHN] (@fromUtc datetime, @toUtc datetime, @force bit = 0)
as
begin
    set nocount on;
    ...
    begin
        ...
        from
            [data].[APC_ATS_CountingReport_AHN] [c]
        where
    ...
    from
        ...
        cross apply (
            select
                [DateTimeLocal] = dateadd(second, 0 , cast([c].[DateTime] at time zone 'UTC' at time zone 'Central European Standard Time' as datetime2(3)))
        ) calc
        join [data].[RT_VehicleJourneyAssignment] [vja] on
            vja.[VehicleNumber] = try_cast(c.[VehicleRef] as int)
